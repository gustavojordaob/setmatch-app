...
exports.resolverPagamentoAsaas = resolverPagamentoAsaas;
exports.montarDescricaoAsaas = montarDescricaoAsaas;
exports.aplicarDescricaoCobrancaAsaas = aplicarDescricaoCobrancaAsaas;
exports.resolverNetValueAsaas = resolverNetValueAsaas;
 ...
 */
function liquidoParaDono(pag, extra) {
    ...
    if (!(bruto > 0))
        return 0;
    return (0, asaasClient_1.netToDono)(bruto);
}
function isAsaasPaymentId(id) {
    return String(id || '').startsWith('pay_');
}
/** Busca cobrança Asaas por pay_id, externalReference ou checkoutSession. */
async function resolverPagamentoAsaas(input) {
    const pick = (p) => {
    ...
    try {
        ...
        if (pid && isAsaasPaymentId(pid) && !pid.startsWith('manual_')) {
            const pay = await (0, asaasClient_1.asaasFetch)(`/v3/payments/${encodeURIComponent(pid)}`);
            const hit = pick(pay);
            if (hit)
                return hit;
        }
    ...
    if (checkoutId) {
        try {
            ...
            const paid = (list.data || []).find((p) => String(p.checkoutSession || '') === checkoutId &&
...
function montarDescricaoAsaas(pag, pagamentoId) {
    const titulo = String(pag.titulo || pag.torneioNome || pag.rankingNome || pag.aulaTitulo || pag.tipo || 'Pagamento').trim();
...
async function aplicarDescricaoCobrancaAsaas(asaasPaymentId, pag, pagamentoId) {
    ...
    const description = String(pag.descricaoAsaas || '').trim() ||
        montarDescricaoAsaas({ ...pag, professorNome }, pagamentoId);
    if (!description)
        return;
    try {
        await (0, asaasClient_1.asaasFetch)(`/v3/payments/${encodeURIComponent(payId)}`, {
            method: 'PUT',
            body: { description },
        });
    }
...
async function resolverNetValueAsaas(input) {
 ...
 */
function saldoJaLiberadoParaSaque(extra) {
    ...
    if (billing === 'PIX' || billing === 'pix')
        return true;
    if (billing === 'CREDIT_CARD' || billing === 'cartao')
        return false;
    // Liberação manual / checkout sem detalhe
    if (event === 'CHECKOUT_PAID') {
        if (String(extra?.meioPagamento || '').toLowerCase() === 'pix')
            return true;
        if (String(extra?.meioPagamento || '').toLowerCase() === 'cartao')
            return false;
        // Checkout ambíguo (PIX+cartão): assume a liberar até PAYMENT_RECEIVED
        return false;
    }
    if (String(extra?.asaasPaymentId || '').startsWith('manual_'))
        return true;
    return true;
}
function meioFromBilling(billing, fallback) {
    const b = String(billing || fallback || '').toUpperCase();
    if (b === 'PIX' || b === 'pix')
        return 'pix';
    if (b === 'CREDIT_CARD' || b === 'DEBIT_CARD' || b === 'cartao')
        return 'cartao';
    return null;
}
/**
 ...
 */
async function creditarSaldoDono(pag, pagamentoId, extra) {
    ...
    const modalidade = String(pag.modalidadeNome || pag.aulaTitulo || pag.torneioNome || pag.rankingNome || tipo).trim() ||
        tipo;
    const meio = meioFromBilling(extra?.billingType, extra?.meioPagamento) ||
        meioFromBilling(pag.meioPagamento) ||
        null;
    const anticipated = extra?.anticipated === true ||
        String(extra?.asaasEvent || '').toUpperCase() === 'PAYMENT_ANTICIPATED';
    const estimatedCreditDate = extra?.estimatedCreditDate != null ? String(extra.estimatedCreditDate) : null;
    ...
    await db().runTransaction(async (tx) => {
        ...
        if (curPag.saldoCreditado === true) {
            ...
            tx.set(saldoRef, {
                ...
                saldoALiberar: firestore_1.FieldValue.increment(-valorMove),
                saldoDisponivel: firestore_1.FieldValue.increment(valorMove),
                atualizadoEm: firestore_1.FieldValue.serverTimestamp(),
            }, { merge: true });
            tx.update(pagRef, {
                liberadoParaSaque: true,
        ...
        const porTipo = { ...(cur.porTipo || {}) };
        porTipo[tipo] = Math.round(((porTipo[tipo] || 0) + liquido) * 100) / 100;
        const saldoPatch = {
            donoUid,
            valoresEmLiquido: true,
            saldoTotalRecebido: firestore_1.FieldValue.increment(liquido),
            porModalidade,
            porTipo,
            atualizadoEm: firestore_1.FieldValue.serverTimestamp(),
        };
        if (liberado) {
            saldoPatch.saldoDisponivel = firestore_1.FieldValue.increment(liquido);
        }
        else {
            saldoPatch.saldoALiberar = firestore_1.FieldValue.increment(liquido);
        }
...
async function liberarInscricaoTorneioAposPagamento(input) {
    ...
    for (const d of inscritosSnap.docs) {
        ...
        const completa = Boolean(insc.nome) ||
    ...
    for (const d of again.docs) {
        ...
        if (compCat === 'dupla' && (!insc.parceiroAceito || !insc.parceiroUid))
            continue;
        const capitaoPago = !precisaPagar || Boolean(insc.pago);
        const parceiroPago = compCat !== 'dupla' || !precisaPagar || Boolean(insc.parceiroPago);
...
async function syncPagamentoAprovado(pagamentoId, paymentId, status, extra) {
    ...
    try {
        ...
        const checkoutId = String(extra?.asaasCheckoutId || pag.asaasCheckoutId || '').trim() ||
        ...
        if (!(Number.isFinite(net) && net > 0) || netSuspeito || checkoutId) {
            ...
            if (resolved)
                net = resolved.netValue;
        }
        const creditExtra = { ...(extra || {}) };
        ...
        if (resolved) {
            ...
            if (resolved.billingType)
                creditExtra.billingType = resolved.billingType;
            if (resolved.anticipated)
                creditExtra.anticipated = true;
            if (resolved.estimatedCreditDate) {
                creditExtra.estimatedCreditDate = resolved.estimatedCreditDate;
            }
            if (resolved.creditDate)
                creditExtra.creditDate = resolved.creditDate;
        ...
        const payIdForDesc = String(creditExtra.asaasPaymentId || resolved?.id || payIdHint || '');
        if (payIdForDesc.startsWith('pay_')) {
            void aplicarDescricaoCobrancaAsaas(payIdForDesc, { ...pag, ...patch, ...creditExtra }, pagamentoId);
        }
    }
    catch (e) {
