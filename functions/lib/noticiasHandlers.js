...
exports.atualizarNoticiasManual = exports.atualizarNoticias = void 0;
exports.sincronizarNoticias = sincronizarNoticias;
...
function googleNewsRss(query, idioma) {
...
function feedsExtras(esporte, idioma) {
    if (idioma === 'en') {
        if (esporte === 'tenis')
            return ['https://feeds.bbci.co.uk/sport/tennis/rss.xml'];
        if (esporte === 'padel')
            return ['https://www.padeladdict.com/feed/'];
        const sub = {
            tenis: 'tennis',
            padel: 'padel',
        ...
        if (reddit)
            return [`https://www.reddit.com/r/${reddit}/hot/.rss?limit=15`];
    }
    return [];
}
function feedsFor(esporte, idioma) {
    return [
    ...
    while ((m = re.exec(xml)) != null) {
        ...
        if (!fonte) {
            if (url.includes('bbc.'))
                ...
                fonte = 'Reddit';
            else if (url.includes('padeladdict'))
                fonte = 'Padel Addict';
            else
                fonte = 'Google News';
        }
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            const resp = await fetch(url, {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (compatible; RallyUpBot/1.0; +https://setmatch-app-fabrica.web.app)',
            ...
            if (!resp.ok)
                throw new Error(`RSS ${resp.status}`);
...
async function coletarItens(urls, limite) {
    const seen = new Set();
    const items = [];
    const erros = [];
...
function docId(esporte, idioma, titulo) {
...
async function sincronizarNoticias() {
            ...
            try {
                ...
                for (const item of items) {
                    ...
                    await db()
                        ...
                        url: item.url,
                        publicadoEm: item.publicadoEm || firestore_1.FieldValue.serverTimestamp(),
                        atualizadoEm: agora,
                        origem: 'rss_auto',
                    }, { merge: true });
                    gravados += 1;
    ...
    for (const docSnap of antigas.docs) {
        ...
        const atualizado = data.atualizadoEm?.toDate?.();
    ...
    await db().doc('config/noticiasSync').set({
        ultimaSyncEm: firestore_1.FieldValue.serverTimestamp(),
        porChave,
        erros,
        deletes,
    }, { merge: true });
...
exports.atualizarNoticias = (0, scheduler_1.onSchedule)({
    schedule: '0 8,20 * * *',
...
exports.atualizarNoticiasManual = (0, https_1.onRequest)({ cors: true, region: 'southamerica-east1', timeoutSeconds: 300, memory: '512MiB' }, async (req, res) => {
...
//# sourceMappingURL=noticiasHandlers.js.map