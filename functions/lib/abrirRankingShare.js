...
exports.paginaAbrirRanking = void 0;
...
/** Página ponte do share de ranking — og + deep link setmatch://ranking/{id}. */
exports.paginaAbrirRanking = (0, https_1.onRequest)({ cors: true, region: 'southamerica-east1', invoker: 'public' }, async (req, res) => {
    const q = typeof req.query.id === 'string' ? req.query.id.trim() : '';
    const pathParts = String(req.path || '')
        .split('/')
        .filter(Boolean);
    const pathId = pathParts[0] === 'abrir' && pathParts[1] === 'ranking' && pathParts[2]
        ? decodeURIComponent(pathParts[2])
        : pathParts[0] === 'ranking' && pathParts[1]
            ? decodeURIComponent(pathParts[1])
            : '';
    const id = q || pathId;
    let nome = 'Ranking no Rally Up';
    let clube = '';
    let banner = DEFAULT_OG;
    if (id) {
        try {
            const snap = await (0, firestore_1.getFirestore)().doc(`rankings/${id}`).get();
            if (snap.exists) {
                const d = snap.data() || {};
                ...
                if (typeof d.clubeLogoUrl === 'string' && d.clubeLogoUrl.startsWith('http')) {
                    banner = d.clubeLogoUrl;
                }
            }
        }
        catch (e) {
            console.warn('paginaAbrirRanking firestore', e);
        }
    }
    const titulo = clube ? `${nome} · ${clube}` : nome;
    const deep = id ? `setmatch://ranking/${encodeURIComponent(id)}` : '';
    const desc = id
        ? 'Abra no app Rally Up para ver a classificação e pedir entrada. Sem app? Baixe na loja.'
        : 'Link inválido — peça um novo link do ranking.';
...
<body>
  <main class="card">
    ...
    <p class="hint">Rally Up não abre o ranking no navegador.</p>
  </main>
  <script>
    (function(){
...
//# sourceMappingURL=abrirRankingShare.js.map