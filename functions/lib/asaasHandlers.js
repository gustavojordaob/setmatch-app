...
exports.consultarSaldoFinanceiro = exports.liberarPagamentoDono = exports.transferirSaldoPix = exports.salvarChavePixDono = exports.webhookAsaasSetmatch = exports.confirmarCheckoutAsaas = exports.criarCheckoutAsaas = void 0;
const firestore_1 = require("firebase-admin/firestore");
const auth_1 = require("firebase-admin/auth");
const https_1 = require("firebase-functions/v2/https");
const asaasClient_1 = require("./asaasClient");
 ...
 */
exports.criarCheckoutAsaas = (0, https_1.onRequest)(FN_OPTS, async (req, res) => {
    if (req.method !== 'POST') {
    ...
    try {
        ...
        const name = String(clubeNome ? `${String(titulo).slice(0, 18)} · ${clubeNome}`.slice(0, 30) : titulo).slice(0, 30);
        const body = {
            billingTypes,
            ...
            items: [
                {
                    ...
                    imageBase64: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5TkJggg==',
                },
            ],
        };
        if (isSubscription) {
            body.subscription = {
                cycle: 'MONTHLY',
                nextDueDate: (0, asaasClient_1.todayISO)(),
            };
        }
        ...
        const cpfCnpj = String(pag.cpfCnpj || pag.cpf || req.body?.cpfCnpj || '').replace(/\D/g, '');
        let email = String(pag.email || '').trim().toLowerCase() ||
        ...
        const checkout = await (0, asaasClient_1.asaasFetch)('/v3/checkouts', { method: 'POST', body });
        const url = checkout.link ||
            `${(0, asaasClient_1.asaasCheckoutHost)()}/checkoutSession/show?id=${encodeURIComponent(checkout.id)}`;
        await pagRef.update({
            provedor: 'asaas',
...
exports.confirmarCheckoutAsaas = (0, https_1.onRequest)(FN_OPTS, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Método não permitido' });
        return;
    }
    try {
        ...
        if (asaasCheckoutId) {
            try {
                const ch = await (0, asaasClient_1.asaasFetch)(`/v3/checkouts/${encodeURIComponent(asaasCheckoutId)}`);
                if (String(ch.status).toUpperCase() === 'PAID') {
                    await (0, pagamentosSync_1.syncPagamentoAprovado)(String(pagamentoId), asaasCheckoutId, 'approved', {
                        provedor: 'asaas',
                        asaasCheckoutId,
                    });
                    res.json({ status: 'aprovado', provedor: 'asaas' });
        ...
        try {
            const list = await (0, asaasClient_1.asaasFetch)('/v3/payments', {
                query: { externalReference: String(pagamentoId), limit: '10' },
            });
            const paid = (list.data || []).find((p) => ['RECEIVED', 'CONFIRMED', 'RECEIVED_IN_CASH'].includes(String(p.status).toUpperCase()));
            if (paid) {
                const net = Number(paid.netValue);
                const statusUp = String(paid.status).toUpperCase();
                const eventHint = statusUp === 'RECEIVED' || statusUp === 'RECEIVED_IN_CASH'
                    ? 'PAYMENT_RECEIVED'
                    : 'PAYMENT_CONFIRMED';
                await (0, pagamentosSync_1.syncPagamentoAprovado)(String(pagamentoId), paid.id, 'approved', {
                    provedor: 'asaas',
...
function mapAsaasPaymentStatus(status) {
    ...
    if (['RECEIVED', 'CONFIRMED', 'RECEIVED_IN_CASH'].includes(s))
        return 'approved';
    if (['REFUNDED', 'REFUND_REQUESTED', 'CHARGEBACK_REQUESTED'].includes(s))
        return 'rejected';
    if (['DELETED', 'OVERDUE'].includes(s))
        return 'cancelled';
    return 'pending';
}
/** Webhook Asaas — configure no painel para esta URL. */
exports.webhookAsaasSetmatch = (0, https_1.onRequest)(FN_OPTS, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Método não permitido' });
        return;
    }
    // Asaas envia o authToken no header `asaas-access-token`
    const expected = asaasClient_1.asaasWebhookToken.value()?.trim();
    if (expected) {
    ...
    try {
        ...
        const payment = (req.body?.payment || req.body?.checkout || {});
        if (event.startsWith('CHECKOUT_')) {
            const checkoutId = String(payment.id || req.body?.checkout?.id || '');
            const externalReference = String(payment.externalReference || req.body?.checkout?.externalReference || '');
            ...
            if (event === 'CHECKOUT_PAID') {
                const netFromBody = Number(payment.netValue ??
                ...
                const billingType = payment.billingType
                    ...
                    : req.body?.payment?.billingType
                        ...
                        : null;
                await (0, pagamentosSync_1.syncPagamentoAprovado)(pagamentoId, checkoutId || pagamentoId, 'approved', {
                    provedor: 'asaas',
                ...
                await (0, pagamentosSync_1.syncPagamentoAprovado)(pagamentoId, checkoutId || pagamentoId, 'cancelled', {
                    provedor: 'asaas',
                    asaasEvent: event,
                });
            }
        ...
        if (event.startsWith('PAYMENT_') || event.startsWith('SUBSCRIPTION_')) {
            ...
            if (!pagamentoId && checkoutSession) {
                const byCheckout = await db()
                    ...
                    .get();
                if (!byCheckout.empty)
                    pagamentoId = byCheckout.docs[0].id;
            }
            ...
            if (event === 'PAYMENT_CREATED' ||
                ...
                if (pagSnap.exists && payId.startsWith('pay_')) {
                    await (0, pagamentosSync_1.aplicarDescricaoCobrancaAsaas)(payId, pagSnap.data() || {}, pagamentoId);
            ...
            if (event === 'PAYMENT_RECEIVED' ||
                ...
                await (0, pagamentosSync_1.syncPagamentoAprovado)(pagamentoId, payId || pagamentoId, 'approved', {
                    provedor: 'asaas',
                ...
                await (0, pagamentosSync_1.syncPagamentoAprovado)(pagamentoId, payId || pagamentoId, mapped, {
                    provedor: 'asaas',
                    asaasPaymentId: payId || null,
                    asaasEvent: event,
                });
            ...
            if ((event === 'TRANSFER_FAILED' || event === 'TRANSFER_CANCELLED') && !snap.data()?.recreditado) {
                ...
                if (donoUid && valor > 0) {
                    await db()
                        ...
                        .doc(donoUid)
                        .set({
                        saldoDisponivel: firestore_1.FieldValue.increment(valor),
                        saldoTransferido: firestore_1.FieldValue.increment(-valor),
                        atualizadoEm: firestore_1.FieldValue.serverTimestamp(),
                    }, { merge: true });
...
async function asaasBalanceCents() {
    try {
        const bal = await (0, asaasClient_1.asaasFetch)('/v3/finance/balance');
...
function msgErroTransferenciaAsaas(raw, asaasCents) {
    const low = raw.toLowerCase();
    if (low.includes('saldo insuficiente')) {
        const asaasTxt = asaasCents != null
            ? ` Saldo liberado na Asaas agora: R$ ${(asaasCents / 100).toFixed(2)}.`
            : '';
        return ('O valor já está no seu extrato do app, mas a conta Asaas da plataforma ' +
...
async function resolveDonoFinanceiroUid(uid) {
    try {
        const uSnap = await db().collection('usuarios').doc(uid).get();
        const d = uSnap.data() || {};
...
exports.salvarChavePixDono = (0, https_1.onRequest)(FN_OPTS, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Método não permitido' });
        return;
    }
    try {
        ...
        const tipo = String(pixTipo || '').toUpperCase();
 ...
 */
exports.transferirSaldoPix = (0, https_1.onRequest)(FN_OPTS, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Método não permitido' });
        return;
    }
    try {
        ...
        const transferirTudo = tudo === true || tudo === 'true' || String(valor || '').toLowerCase() === 'tudo';
        ...
        const result = await db().runTransaction(async (tx) => {
            ...
            if (pedirCents > disponivelCents) {
                throw new Error(`Saldo insuficiente no app. Disponível: R$ ${(disponivelCents / 100).toFixed(2)}`);
            }
            if (pedirCents < 100) {
                throw new Error('Valor mínimo R$ 1,00');
            }
            if (asaasCents != null && pedirCents > asaasCents) {
                throw new Error(msgErroTransferenciaAsaas('Saldo insuficiente para realizar a operação.', asaasCents));
            }
            const amount = pedirCents / 100;
            const tipo = String(pixTipo || data.pixTipo || 'EVP').toUpperCase();
            ...
            tx.set(saldoRef, {
                donoUid,
                saldoDisponivel: firestore_1.FieldValue.increment(-amount),
                saldoTransferido: firestore_1.FieldValue.increment(amount),
                pixTipo: tipo,
                pixChave: chave,
                atualizadoEm: firestore_1.FieldValue.serverTimestamp(),
            }, { merge: true });
        ...
        try {
            const transfer = await (0, asaasClient_1.asaasFetch)('/v3/transfers', {
                method: 'POST',
                body: {
                    value: amount,
            ...
            await db()
                ...
                status: String(transfer.status).toUpperCase() === 'DONE' ? 'concluida' : 'pendente',
                atualizadoEm: firestore_1.FieldValue.serverTimestamp(),
            });
            res.json({
                ok: true,
...
exports.liberarPagamentoDono = (0, https_1.onRequest)(FN_OPTS, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Método não permitido' });
        return;
    }
    try {
        ...
        await (0, pagamentosSync_1.syncPagamentoAprovado)(String(pagamentoId), `manual_${pagamentoId}`, 'approved', {
...
exports.consultarSaldoFinanceiro = (0, https_1.onRequest)(FN_OPTS, async (req, res) => {
...
//# sourceMappingURL=asaasHandlers.js.map