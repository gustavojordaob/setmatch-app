    ...
    try {
        ...
        try {
            ...
            if (meioNorm === 'pix' && /pix/i.test(msg)) {
                ...
                res.status(400).json({
                    error: 'PIX ainda não está liberado nesta conta Stripe (invite-only no Brasil). Escolha Cartão no próximo passo, ou ative o PIX no Dashboard Stripe → Pagamentos → Formas de pagamento.',
