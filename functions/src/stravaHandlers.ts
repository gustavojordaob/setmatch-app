...
export const stravaSyncToday = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const activities = (await resp.json()) as { kilojoules?: number; calories?: number }[];
      let kcal = 0;
      for (const a of activities) {
        if (typeof a.calories === 'number') kcal += a.calories;
        else if (typeof a.kilojoules === 'number') kcal += a.kilojoules / 4.184;
      }
