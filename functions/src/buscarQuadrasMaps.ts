 ...
 */
export const buscarQuadrasMaps = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (queryTxt.length >= 2) {
        ...
        const batches = await Promise.all(
          keywords.map((keyword) =>
            nearbySearch({ key, lat, lng, radiusM, keyword }).catch(() => [] as GooglePlace[])
          )
        );
        raw = batches.flat();
      }

      const byId = new Map<string, PlaceOut>();
      for (const p of raw) {
        const mapped = mapPlace(p, lat, lng);
        if (!mapped || mapped.distanciaKm > raioKm) continue;
      ...
      const lugares = [...byId.values()].sort(
